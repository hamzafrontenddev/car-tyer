...
const ProfitLoss = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeReturn = onSnapshot(collection(db, "returnedTyres"), (snapshot) => {
      const data = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
      console.log("Return Data Updated:", data); // Debug return data
      setReturnData(data);
    });
  ...
  const totalReturnAmount = filteredReturnData.reduce(
    (sum, item) => sum + (parseFloat(item.returnPrice || 0) * parseInt(item.returnQuantity || 0, 10) || 0),
    0
  );
  const netSales = totalSalesRevenue - totalReturnAmount;
  const grossProfit = netSales - totalPurchaseCost;
  const profitMargin = totalPurchaseCost ? ((grossProfit / totalPurchaseCost) * 100).toFixed(2) : 0;
  ...
  filteredBuyData.forEach((item) => {
    ...
    brandBreakdown[item.brand].cost += parseFloat(item.price || 0) * parseInt(item.quantity || 0, 10) || 0;
  });
  filteredSellData.forEach((item) => {
    ...
    brandBreakdown[item.brand].revenue += parseFloat(item.price || 0) * parseInt(item.quantity || 0, 10) || 0;
  });
  filteredReturnData.forEach((item) => {
    if (!brandBreakdown[item.brand]) {
      brandBreakdown[item.brand] = { soldQty: 0, revenue: 0, cost: 0, returns: 0 };
    }
    brandBreakdown[item.brand].returns += parseFloat(item.returnPrice || 0) * parseInt(item.returnQuantity || 0, 10) || 0;
  });
  ...
  const tyreProfit = filteredSellData.map((item) => {
    ...
    const cost = buyItem ? parseFloat(buyItem.price || 0) * parseInt(item.quantity || 0, 10) : 0;
    const revenue = parseFloat(item.price || 0) * parseInt(item.quantity || 0, 10) || 0;
    const returns = returnItems.reduce((sum, r) => sum + (parseFloat(r.returnPrice || 0) * parseInt(r.returnQuantity || 0, 10) || 0), 0);
  ...
  const totalReturnedQty = filteredReturnData.reduce((sum, item) => sum + parseInt(item.returnQuantity || 0, 10) || 0, 0);
  const returnByBrand = {};
  filteredReturnData.forEach((item) => {
    if (!returnByBrand[item.brand]) returnByBrand[item.brand] = 0;
    returnByBrand[item.brand] += parseInt(item.returnQuantity || 0, 10) || 0;
  });
  const mostReturnedBrand = Object.keys(returnByBrand).reduce((a, b) => (returnByBrand[a] > returnByBrand[b] ? a : b), "N/A");

  const totalBought = filteredBuyData.reduce((sum, item) => sum + parseInt(item.quantity || 0, 10) || 0, 0);
  const totalSold = filteredSellData.reduce((sum, item) => sum + parseInt(item.quantity || 0, 10) || 0, 0);
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 bg-gray-50 min-h-screen">
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {brandBreakdownArray.map((row, idx) => (
                <tr key={row.brand} className={`border-b ${idx % 2 === 0 ? "bg-gray-50" : "bg-white"} hover:bg-gray-100 transition-colors`}>
      ...
      <div className="bg-white shadow-lg rounded-xl p-6 mb-8 border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {tyreProfit.map((tyre, idx) => (
            ...
            >
              <p className="text-sm font-semibold text-gray-800">{tyre.brand} - {tyre.model}</p>
              <p className="text-xs text-gray-600">Size: {tyre.size}</p>
              <p className="text-xs text-gray-600">Units Sold: {tyre.unitsSold}</p>
              <p className="text-lg font-bold text-green-600 mt-2">Profit: Rs. {tyre.profit.toLocaleString()}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white shadow-lg rounded-xl p-6 mb-8 border border-gray-100">
        <h3 className="text-xl font-semibold text-gray-800 mb-4">E. Stock Movement Overview</h3>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="p-4 bg-blue-50 rounded-lg shadow-sm">
            <p className="text-sm text-gray-600">📥 Total Bought</p>
            <p className="text-lg font-bold text-blue-700">{totalBought}</p>
          </div>
          ...
          <div className="p-4 bg-yellow-50 rounded-lg shadow-sm">
            <p className="text-sm text-gray-600">🏷️ In Stock</p>
            <p className="text-lg font-bold text-yellow-700">{inStock}</p>
          </div>
        </div>
      </div>

      <div className="bg-white shadow-lg rounded-xl p-6 border border-gray-100">
        <h3 className="text-xl font-semibold text-gray-800 mb-4">F. Business Health Indicators</h3>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="p-4 bg-teal-50 rounded-lg shadow-sm">
            <p className="text-sm text-gray-600">🔄 Stock Turnover Rate</p>
            <p className="text-lg font-bold text-teal-700">{stockTurnoverRate}%</p>
            <p className="text-xs text-gray-500">Sold out of total bought</p>
          </div>
          <div className="p-4 bg-orange-50 rounded-lg shadow-sm">
            <p className="text-sm text-gray-600">📉 Return Rate</p>
            <p className="text-lg font-bold text-orange-700">{returnRate}%</p>
            <p className="text-xs text-gray-500">Returned out of sold tyres</p>
          </div>
          <div className="p-4 bg-purple-50 rounded-lg shadow-sm">
            <p className="text-sm text-gray-600">💰 Avg. Profit per Tyre</p>
            <p className="text-lg font-bold text-purple-700">Rs. {avgProfitPerTyre}</p>
          </div>
          <div className="p-4 bg-green-50 rounded-lg shadow-sm">
            <p className="text-sm text-gray-600">🏆 Best Brand (Profit %)</p>
            <p className="text-lg font-bold text-green-700">{bestBrand.brand}</p>
            <p className="text-xs text-gray-500">{bestBrand.margin}%</p>
          </div>
        </div>
      </div>
    </div>
